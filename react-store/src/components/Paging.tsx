    ...
    <div>
        <ul className="pagination">
            ...
            <li className={`page-item ${current_page <= 1? 'disabled':''}`} aria-label="&laquo; Previous">
                <Link to={''} className={`page-link ${current_page <= 1? 'disabled':''}`} onClick={() => setPage(current_page-1)} rel="prev" aria-label="Prev &laquo;">&lsaquo;</Link>
            </li>
        
            {Array.from({length: totalPages}, (_, i) => i + 1).map((page) => (
                <li className={`page-item ${current_page === page ? 'active' : ''}`} key={page}>
                    <Link to={''} className="page-link" onClick={() => setPage(page)}>{page}</Link>
                </li>
            ))}
        
            <li className={`page-item ${current_page >= totalPages? 'disabled':''}`}>
                <Link to={''} className={`page-link ${current_page >= totalPages? 'disabled':''}`} onClick={() => setPage(current_page+1)} rel="next" aria-label="Next &raquo;">&rsaquo;</Link>
            </li>
            </ul>
            </div>
        </div>
